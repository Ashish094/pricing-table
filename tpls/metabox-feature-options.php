<script type="text/javascript" src="<?php echo theme_url('/inc'); ?>/pricing-table/js/admin/jquery.tablednd_0_5.js"></script>
<script>
    ...
    jQuery('body').on('click', '.featured-package', function() {
        ...
        jQuery('.featured-package').attr('src','<?php echo theme_url('/inc'); ?>/pricing-table/images/unfeatured.png');
        if(isf!="<?php echo theme_url('/inc'); ?>/pricing-table/images/featured.png") {
            jQuery('#'+fid).attr('src',"<?php echo theme_url('/inc'); ?>/pricing-table/images/featured.png");
 ...
 <table id="pricetable" class="draggable widefat" border="0" width="100%" cellspacing="0" cellpadding="0" >
   ...
   <tr class="nodrag nodrop">
    ...
    foreach($package_name as $index=> $value){
        ...
        echo  '<td class="'.$index.'"><img src="'. theme_url('/inc').'/pricing-table/images/delete.png" class="deletecol" rel="'.$index.'" title="Delete this column" />
        <strong><span id="sp'.$index.'">
        '.$value.'
        </span>
        </strong>
        <input type="hidden" name="package_name['.$index.']" id="val_'.$index.'" value="'.$value.'" />
        <img rel="'.$value.'"  id="f'.$imgc.'" class="featured-package" title="click here to feature" src="'. theme_url('/inc').'/pricing-table/images/'.$fimg.'">
      <img rel="sp'.$index.'"  id="e'.$index.'" class="featured-package-edit" title="click here to edit" src="'. theme_url('/inc').'/pricing-table/images/edit.png"  />
      </td>';
    }
    }
?>
        ...
        foreach($package_name as $index => $value){
            ...
            if($t==1){
                ...
                if($value1 != "Price" && $value1!="Detail" && $value1!="ButtonURL" && $value1!="ButtonText" ){
                    ...
                    echo '<img class="rdndHandler" src="'.theme_url('/inc').'/pricing-table/images/updown.png" />';
                    echo '<img rel="sf'.$index1.'"  id="e'.$index1.'" class="feature-edit" title="click here to edit Feature Name" src="'. theme_url('/inc').'/pricing-table/images/edit.png"> ';
                    echo '<img rel="sfd'.$index1.'"  id="ed'.$index1.'" class="feature-desc-edit" title="click here to edit Feature Description" src="'. theme_url('/inc').'/pricing-table/images/edit-desc.png"> ';
                    echo "<img src='". theme_url('/inc')."/pricing-table/images/delete.png' class='deleterow' rel='{$index1}' title='Delete this row' /><br/>";
                }
    ...
    jQuery(function(){
      jQuery('#addrow').click(function(){
          ...
          $ftr_info = "<span id='sf"+tmp_fid+"'>"+feat+"</span><br/><input type='hidden' name='feature_name["+tmp_fid+"]' value='"+feat+"'/><input type=hidden id='val_desc_"+tmp_fid+"' class='d' placeholder='Tooltip Text' name='feature_description["+tmp_fid+"]' value=''/>";
           ...
           jQuery('#pricetable tbody tr:last td:first').html('<img class="rdndHandler" src="<?php echo theme_url('/inc');?>/pricing-table/images/updown.png" /><img rel="sf'+tmp_fid+'"  id="e'+tmp_fid+'" class="feature-edit" title="click here to edit Feature Name" src="<?php echo theme_url('/inc');?>/pricing-table/images/edit.png"><img rel="val_desc_'+tmp_fid+'"  id="ed'+tmp_fid+'" class="feature-desc-edit" title="Click here to edit Feature Description" src="<?php echo theme_url('/inc');?>/pricing-table/images/edit-desc.png"><img src="<?php echo theme_url('/inc');?>/pricing-table/images/delete.png" class="deleterow" title="Delete this row" /><br/>');
      ...
      jQuery('#addcolumn').click(function(){
          ...
          if(trim(jQuery('#pricetable tbody tr:last td:first').html())=="Packages/Features"){
              ...
              jQuery("#pricetable").find("tr").each(function() {
               ...
               jQuery('#pricetable tbody tr:first td:last').append("<img class='deletecol' rel='"+tmp_pid+"' title='Delete this row' src='<?php echo theme_url('/inc');?>/pricing-table/images/delete.png' />");
               jQuery('#pricetable tbody tr:first td:last').append('<img rel="'+package+'"  id="f'+tmp_pid+'" class="featured-package" title="click here to feature" src="<?php echo theme_url('/inc');?>/pricing-table/images/unfeatured.png" > <img rel="sp'+tmp_pid+'"  id="e'+tmp_pid+'" class="featured-package-edit" title="click here to edit" src="<?php echo theme_url('/inc');?>/pricing-table/images/edit-desc.png"> ');
