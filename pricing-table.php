...
Plugin URI: http://www.kleis.it
Description: WordPress Plugin for creating simple pricing tables
Author: Kleis
Version: 0.9
...
function wppt_custom_init()
{
    ...
    $args = array(
        ...
        'menu_icon' => theme_url().'/inc/pricing-table/images/table.gif'
    );
...
function wppt_preview_table($content){
    ...
    <form action="">
        ...
        <?php if(isset($styles[$template]) && $styles[$template]>0){ ?>
            <b>Select Style</b><br/>
            <select onchange="jQuery('#ptts').attr('href','<?php echo theme_url('/inc/pricing-table/table-templates/'.$template.'/style');?>'+this.value+'.css')">
    ...
    ob_clean();
    $code[] = "[y]";
    $icons[] = "<img src='".theme_url("/inc/pricing-table/images/yes.png")."' />";
    $code[] = "[n]";
    $icons[] = "<img src='".theme_url("/inc/pricing-table/images/no.png")."' />";
    $code[] = "[na]";
    $icons[] = "<img src='".theme_url("/inc/pricing-table/images/na.png")."' />";
...
function wppt_table($params){
    ...
    $template = isset($params['template'])?$params['template']:'smooth';
    $responsive = isset($params['responsive'])?'responsive':'';
    ob_start();
    include("table-templates/{$template}/price_table.php");
    $data = ob_get_contents();
...
function wppt_column_obj( $column ) {
    ...
    switch ( $column ) {
        case 'shortcode':
            echo "<input type=text readonly=readonly value='[kleis-pricing-table id={$post->ID}]' size=25 style=\"box-shadow:none;border:1px solid #ddd;text-align:Center;font-family:'Courier New';font-size:10pt;pading:5px 10px;\" onclick='this.select()' />";
            break;
    }}
}
...
function wppt_admin_enqueue_scripts(){
    ...
    wp_enqueue_script('wppt-datatable', theme_url('/inc/pricing-table/js/admin/dragtable.js'));
    wp_enqueue_script('wppt-tablednd', theme_url('/inc/pricing-table/js/admin/jquery.tablednd_0_5.js'));
    wp_enqueue_style("wppt-my", theme_url('/inc/pricing-table/css/admin/my.css'));
    wp_enqueue_style("wppt-tablestyle", theme_url('/inc/pricing-table/css/admin/tablestyle.css'));
    load_scripts();
    load_styles();

    //wp_enqueue_script("tiptipjs", theme_url()."/inc/pricing-table/js/site/jquery.tipTip.minified.js",array('jquery'));
    //wp_enqueue_style("tiptipcss", theme_url()."/inc/pricing-table/css/site/tipTip.css");
}
...
function wppt_enqueue_scripts(){
    ...
    if($enque==1){
        ...
        //wp_enqueue_script('wppt-tablednd', theme_url('/inc/pricing-table/js/site/icon.js'));
...
function wppt_baseurl(){
    ...
    if($enque==1){
        ?>
        <script language="JavaScript">
            var wppt_url = "<?php echo theme_url('/inc/pricing-table/'); ?>";
        </script>
    <?php
    }
}
...
function wppt_detect_shortcode()
{
    ...
    if (   preg_match_all( '/'. $pattern .'/s', $post->post_content, $matches )
        ...
        && in_array( 'kleis-pricing-table', $matches[2] ) )
    {
        $enque = 1;
    }
}
...
function load_styles(){
    $plugin_dir = get_template_directory() . '/inc/pricing-table';
    $plugin_url = get_template_directory_uri() . '/inc/pricing-table';
    $dir = is_admin()?'admin':'site';
    $cssdir = $plugin_dir.'/css/'.$dir.'/';
    $cssurl = $plugin_url.'/css/'.$dir.'/';
    $files = scandir($cssdir);
    foreach($files as $file){
        $tt = explode(".",$file);
        //echo "<pre>"; var_dump($file); echo "</pre><br/>";
        if($file!='.'&&$file!='..'&&!@is_dir($file)&&end($tt)=='css')
...
function load_scripts(){
    ...
    wp_enqueue_script('jquery');
    $dir = is_admin()?'admin':'site';
    $jsdir = $plugin_dir.'/js/'.$dir.'/';
    $jsurl = $plugin_url.'/js/'.$dir.'/';
    $files = scandir($jsdir);
    foreach($files as $file){
        $tt = explode(".",$file);
        //echo "<pre>"; var_dump($file); echo "</pre><br/>";
        if($file!='.'&&$file!='..'&&!@is_dir($file)&&end($tt)=='js')
...
add_shortcode("kleis-pricing-table",'wppt_table');
...
//include( theme_url('/inc/pricing-table/libs/class.plugin.php') );
//$plugin = new ahm_plugin('pricing-table');
//$plugin->load_styles();
//$plugin->load_scripts();
//$plugin->load_modules();
